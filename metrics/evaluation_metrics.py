...
def one_for_one_EMD_CD_(sample_pcs, ref_pcs, accelerated_cd=True):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    if sample_pcs.shape[1] < 10: 
        sample_pcs = np.swapaxes(sample_pcs, 1, 2).contiguous()
    ...
    for sample_b_start in iterator:
        sample_batch = sample_pcs[sample_b_start].view(1, -1, 3).contiguous()
        ref_batch = ref_pcs[sample_b_start].view(1, -1, 3).contiguous()
        sample_mean, ref_mean = sample_batch.mean(), ref_batch.mean()
        sample_std, ref_std = sample_batch.std(), ref_batch.std()
        sample_batch = (sample_batch - sample_mean) / sample_std * ref_std + ref_mean
        # sample_max, sample_min = sample_batch.max(), sample_batch.min()
        # sample_batch = ((sample_batch - sample_min) / (sample_max - sample_min))*2-1
        if accelerated_cd:
            dl, dr = distChamferCUDA(sample_batch, ref_batch)
        else:
            dl, dr = distChamfer(sample_batch, ref_batch)
        cd_res = dl.mean() + dr.mean()
        emd_batch = emd_approx(sample_batch, ref_batch)
        # print(cd_res, emd_batch)
        all_cd += cd_res 
        all_emd += emd_batch 
    return all_cd / N_sample, all_emd / N_sample

def pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    all_cd = []
    all_emd = []
...
def compute_all_metrics(sample_pcs, ref_pcs, batch_size, accelerated_cd=False):
    ...
    M_rs_cd, M_rs_emd = pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=accelerated_cd)
    ...
    M_rr_cd, M_rr_emd = pairwise_EMD_CD_(ref_pcs, ref_pcs, batch_size, accelerated_cd=accelerated_cd)
    M_ss_cd, M_ss_emd = pairwise_EMD_CD_(sample_pcs, sample_pcs, batch_size, accelerated_cd=accelerated_cd)
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>1:
        ...
        for class_name in ["bed", "bathtub", "desk", "chair", "sofa", "table", "toilet", "monitor", "night_stand", "dresser"]:
            x = np.load("data/%s_train.npy" % class_name)
            x = torch.from_numpy(x)
            y = np.load("output/a_bridge/test/%s_new_rec.npy" % class_name)
            if y.shape[1] < 10: 
                y = np.swapaxes(y, 1, 2)
            y = torch.from_numpy(y)
            print(y.shape)
            a, b = one_for_one_EMD_CD_(y.cuda(), x.cuda())
            print("ebp_new_%s" % class_name, a*100, b*10)
            # y = np.load("../PointCloud_reference/PointFlow/checkpoints/modelnet/%s/flow_rec.npy" % class_name)
            # y = torch.from_numpy(y)
            # y = y[:x.shape[0]]
            # a, b = one_for_one_EMD_CD_(y.cuda(), x.cuda())
            # print("flow_%s" % class_name, a*100, b*10)
        # B, N = 2, 10
        # x = torch.rand(B, N, 3)
        # y = torch.rand(B, N, 3)

    # a, b = one_for_one_EMD_CD_(x.cuda(), y.cuda())
    # print(a, b)
    # print(min_l.shape)
    # print(min_r.shape)
